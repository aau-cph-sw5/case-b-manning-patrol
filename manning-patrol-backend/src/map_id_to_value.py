# Make a function that takes the mapping.json file and returns a dictionary
# with the Id as the key and the Station or Train as the value.
import json
...
def load_mapping():
    ...
    mapping = {}
    for item in data:
        # Validate the item using the pydantic model
        validated_item = BeaconToStation(**item)
...
# Make a function that takes an id of the mapping and returns
# the corresponding Station or Train.
def get_mapping_value(item_id):
    mapping = load_mapping()
    return mapping.get(item_id)


# Make a function that takes get_mapping_value and logs it into a file
# called mapping.log
